    ...
    if (isset($_SESSION['id_usuario'])  && $_SESSION['verifyP2']) {
        ...
        switch ($_GET['action']) {
            ...
            case 'eliminarQRAUTH':
                ...
                break;
                //Consulta para el reporte de los empleados sin accesos
            case 'empleadosSAE':
                if ($result['dataset'] = $empleados->accesoEmpleadosEmp()) {
                    $result['status'] = 1;
                } elseif (Database::getException()) {
                    $result['exception'] = Database::getException();
                } else {
                    $result['exception'] = 'No hay empleados sin acceso';
                }
                break;
                //Consulta para el reporte de los empleados con accesos
            case 'empleadosCAE':
                if ($result['dataset'] = $empleados->accesoEmpleadosEEmp()) {
                    $result['status'] = 1;
                } elseif (Database::getException()) {
                    $result['exception'] = Database::getException();
                } else {
                    $result['exception'] = 'No hay empleados con acceso';
                }
                break;
            default:
