...
class EnvioC
{
    public function enviarEmailI($id,$pdf,$rname)
    {
        ...
        $correo = $rec->obtenerCorreoEmp($id);
        $pdf = $pdf;
        $reportn = $rname;
        $body = '<!DOCTYPE html
        ...
        try {
            ...
            $mail->Password   = 'epmwcvqlxmctvxzh';                                   //SMTP password
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
            $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

            //Recipients
            $mail->setFrom('despcontableesquivel@gmail.com', 'Smart Bookkeeping');
            $mail->addAddress($correo, 'empleado de SmartBook');     //Add a recipient
